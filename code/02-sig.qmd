...
```{r inhouse}
gs <- fromJSON(file.path(DAT, "InHouse.json"))
...
df <- read.csv(file.path(DAT, "tum.csv"))
...
use <- read_xlsx(file.path(DAT, "sig_use.xlsx"))
all <- read.csv(file.path(DAT, "sig_all.csv"))
...
```{r save-data}
saveRDS(res, file.path(OUT, "sig.rds"))
```
