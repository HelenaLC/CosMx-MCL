...
pbs <- aggregateAcrossCells(
    x=logNormCounts(sce), ids=kid[colnames(sce)],
    statistics="mean", use.assay.type="logcounts")
```

```{r plt-pbs, fig.width=28, fig.height=4}
#| code-fold: true
# selection
n <- 10
ks <- colnames(pbs)
...
# remove duplicates
for (. in seq_along(gs)[-1])
    gs[[.]] <- setdiff(gs[[.]], unlist(gs[seq(.-1)]))
# wrangling
ns <- sapply(gs, length)
mx <- .z(t(es[gs <- unlist(gs), ]))
pal <- setNames(.pal_kid[seq_along(ks)], ks)
# plotting
pheatmap::pheatmap(mx, 
    ...
    show_colnames=TRUE, gaps_col=cumsum(ns),  
    annotation_colors=list(x=pal), annotation_names_col=FALSE,
    annotation_col=data.frame(row.names=gs, x=rep.int(colnames(pbs), ns)))
```
