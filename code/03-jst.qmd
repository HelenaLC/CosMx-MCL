...
```{r load-data}
pbs <- readRDS(file.path(OUT, "qbs.rds"))
sce <- readRDS(file.path(OUT, "fil.rds"))
ist <- readRDS(file.path(OUT, "lv1.rds"))
names(sub) <- sub <- sort(unique(ist$clust))
```
...
```{r echo=FALSE}
run <- !file.exists(out <- file.path(OUT, "jst.rds"))
```
...
lapply(setdiff(sub, "tum"), \(.) {
    ...
    n1 <- ifelse(. == "str", 10, 20)
    gs <- c("CD68", "CD79A", "MS4A1", "ITGAX", "CD68")
    ps <- .plt_de(kst, "all", n1=n1, n2=80, gs=gs); names(ps)[1] <- "all"
    for (. in names(ps)) {cat("####",.,"\n"); print(ps[[.]]); cat("\n\n")}
    cat(":::\n")
})
```
...
```{r save-data}
saveRDS(jst, file.path(OUT, "jst.rds"))
saveRDS(kst, file.path(OUT, "lv2.rds"))
```
