...
```{r}
# select
rho <- cor(t(y), sce$fqs$tum)
rho <- rho[order(abs(rho[, 1])), 1]
top <- tail(names(rho), 40)
# average by bin
xs <- seq(-(dx <- 0.05), 1+dx, dx*2)
fq <- cut(.q(sce$fqs$tum), breaks=xs)
fq <- xs[as.integer(fq)]+dx
fq <- factor(fq, sort(unique(fq)))
sub <- sce[rowData(sce)$typ == "pw", ]
y <- as.matrix(Reduce(`+`, assays(sub))/2)
assay(sub, "foo") <- y
...
# selection
mx <- as.matrix(df[, rownames(sub)])
ds <- rowDiffs(colRanges(mx))
ds <- ds[order(abs(ds[, 1])), 1]
gs <- names(tail(ds, 20))
rho <- cor(t(y), sce$fqs$tum)
rho <- rho[order(abs(rho[, 1])), 1]
gs <- names(tail(rho, 40))
# scale across bins
fd <- df |>
    pivot_longer(-fq) |>
    group_by(name) |>
