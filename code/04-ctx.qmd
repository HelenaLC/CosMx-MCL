...
```{r plt-xy, results="asis", fig.align="default", fig.width=10, fig.height=3}
idx <- split(colnames(sce), sce$sid)
pol <- file.path(OUT, "pol.parquet")
...
```{r save-data}
saveRDS(fq, file.path(OUT, "fqs.rds"))
saveRDS(ns, file.path(OUT, "ctx.rds"))
```
